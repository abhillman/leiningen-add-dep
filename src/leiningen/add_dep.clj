...
(defn to-leiningen-dep [-maven-coordinate]
...
(defn read-project []
  (vec (read-string (slurp (io/file "project.clj")))))

(defn get-deps-section-idx [project-as-vec]
...
(defn set-deps-section [deps-section project-as-vec]
  (let [deps-idx (get-deps-section-idx project-as-vec)]
    (if (= deps-idx -1)
      (concat project-as-vec (cons :dependencies (cons deps-section '())))
      (assoc project-as-vec (inc deps-idx) deps-section))))

(defn add-dep [-maven-coordinate project-as-vec]
 ;; (vec nil) := (vec '())
 ;; https://clojuredocs.org/clojure.core/vec
 (set-deps-section
   (-> (vec (get-deps-section project-as-vec))
        (merge (to-leiningen-dep -maven-coordinate))) project-as-vec))
...
;;   (if (not (.exists (io/file "project.clj"))) (err! "project.clj does not exist; exiting.")))
